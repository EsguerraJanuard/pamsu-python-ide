...
new_student_item = """<div 
                ...
                >
                  {selectedStudent?.student_id === student.student_id && (
                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-emerald-500 rounded-r shadow-[0_0_8px_rgba(16,185,129,0.5)] z-10"></div>
                  )}
                  ...
                  <span className={`px-2 py-1 text-xs rounded-full relative z-10 ${badgeColor}`}>
                    {badgeText}
                  </span>
                </div>"""
