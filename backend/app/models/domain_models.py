...
class User(Base):
    ...
    academic_events_created = relationship(
        "AcademicEvent",
        back_populates="actor",
        foreign_keys="AcademicEvent.actor_user_id",
    )
...
class AcademicEvent(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            "'grade'"
            ")",
            name="ck_academic_events_resource_type",
        ),
        UniqueConstraint(
            "event_key",
            name="uq_academic_events_event_key",
        ),
        Index(
            "ix_academic_events_type_occurred",
            "event_type",
            "occurred_at",
        ),
        Index(
            "ix_academic_events_resource",
            "resource_type",
            "resource_id",
        ),
    )
    ...
    actor = relationship(
        "User",
        back_populates="academic_events_created",
    ...
    # Academic-event records represent completed domain actions. Services
    # may create and read these records but must not alter their event type,
    # resource identity, actor, payload, or occurrence timestamp.
    #
    # IDEMPOTENCY BOUNDARY:
    # event_key is generated by the backend from the originating domain
    # action. Retrying the same action must reuse the same event key instead
    # of creating a duplicate academic event.
    #
    # PRIVACY BOUNDARY:
    # event_data may contain only explicitly approved academic metadata.
...
class Notification(Base):
    ...
    # recipient_id is selected by backend domain logic. Clients must never
    # create notifications or choose another user's notification recipient.
    #
    # DUPLICATE-PREVENTION BOUNDARY:
    # One recipient may receive at most one notification for a single
    # academic event.
    #
    # CONTENT BOUNDARY:
    # title and message are generated by trusted backend templates. They
    # must not include source code, hidden test data, analytics details,
    # session telemetry, surveillance information, or unreleased grades.
    #
    # DELIVERY BOUNDARY:
